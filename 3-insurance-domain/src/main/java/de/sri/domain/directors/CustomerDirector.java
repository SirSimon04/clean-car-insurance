...
public class CustomerDirector {
    ...
    public Customer buildTemporary(String firstName, String lastName, LocalDate dateOfBirth, String email,
            Address address) {
        return builder.withId(0).withFirstName(firstName).withLastName(lastName).withDateOfBirth(dateOfBirth)
    ...
    public Customer buildNew(int id, String firstName, String lastName, LocalDate dateOfBirth, String email,
            Address address) {
        return builder.withId(id).withFirstName(firstName).withLastName(lastName).withDateOfBirth(dateOfBirth)
                .withEmail(email).withAddress(address).build();
    }

    public Customer buildNewFromObject(int id, Customer customer) {
        return builder.withId(id).withFirstName(customer.getFirstName()).withLastName(customer.getLastName())
                .withDateOfBirth(customer.getDateOfBirth()).withEmail(customer.getEmail())
